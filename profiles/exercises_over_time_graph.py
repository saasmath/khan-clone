...
class ExerciseData(object):
        def __init__(self, name, exid, days_until_proficient, proficient_date):
            self.name = name
            self.display_name = exercise_models.Exercise.to_display_name(self.name)
            self.exid = exid
...
def exercises_over_time_graph_context(user_data):
    ...
    for ue in exercise_models.UserExercise.all().filter('user =', user_data.user).filter('proficient_date >', None).order('proficient_date'):
