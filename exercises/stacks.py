...
# of a double-pickled list of cards attached to UserTopics or something
# similar.
class Card(object):
    """ Holds single Card's state.
    Subclassed by ProblemCard and, in the future, stuff like
    VideoCard, SurveyCard, etc.
    """
...
def get_problem_stack(next_user_exercises):
    ...
    """
    problem_cards = [ProblemCard(ue.exercise) for ue in next_user_exercises]
    ...
    if HappyPictureCard.should_include():
        ...
        problem_cards = (problem_cards[:ix] +
                [HappyPictureCard()] +
                problem_cards[(ix + 1):])
...
def get_review_stack(next_user_exercises):
    review_cards = [ProblemCard(ue.exercise) for ue in next_user_exercises]
